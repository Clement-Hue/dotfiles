...
local function get_dir_and_prefix(path)
  local dir, prefix = "", path
...
local function get_base_dirs(self, req_type, context)
  if req_type == "require_relative" then
    return { vim.fn.expand(("#%d:p:h"):format(context.bufnr)) }
  else
    return self:get_load_paths(context.bufnr)
  end
end

local function get_items(base_dirs, dir_part, edit_range)
  local seen, items = {}, {}
...
local function set_doc_window_line_numbers(window)
  local winnr = window:get_win()
...
function source:get_completions(context, callback)
  ...
  local dir_part, prefix = get_dir_and_prefix(path_so_far)

  local base_dirs = get_base_dirs(self, req_type, context)
  ...
  local items = get_items(base_dirs, dir_part, edit_range)
    ...
    item.documentation = {
      ...
      draw = function(opts)
        ...
        vim.schedule(function()
          set_doc_window_line_numbers(opts.window)
        end)
      end,
    }
  end
